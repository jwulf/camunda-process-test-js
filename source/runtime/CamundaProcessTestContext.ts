...
export class CamundaProcessTestContext {
  ...
  async deployProcess(resourcePath: string, processId?: string): Promise<void> {
    ...
    debugDeploy('📍 Process Definition Key: %s', deployment.processDefinitionKey);
  ...
  async deployDecision(resourcePath: string): Promise<void> {
    ...
    const response = await zeebe.deployResourcesFromFiles([
     resourcePath
    ]);
  ...
  mockJobWorker(jobType: string): JobWorkerMock {
    ...
    debugWorker('📝 Worker registered (total workers: %d)', this.jobWorkers.length);
    return worker;
  }
   ...
   */
  increaseTime(duration: number | { days?: number; hours?: number; minutes?: number; seconds?: number }): void {
    ...
    debug(`Time increased by ${milliseconds}ms to ${this.currentTime.toISOString()}`);
