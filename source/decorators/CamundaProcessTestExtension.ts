...
export class CamundaProcessTestExtension {
  ...
  async beforeAll(): Promise<void> {
    ...
    const runtimeConfig: CamundaRuntimeConfiguration = ContainerRuntimePropertiesUtil.readProperties();
  ...
  getClient(): Camunda8 {
    if (!this.client) {
      throw new Error('Client not initialized. Make sure the test extension is properly set up.');
    }
    return this.client;
  }
  ...
  getContext(): CamundaProcessTestContext {
    if (!this.context) {
      throw new Error('Context not initialized. Make sure the test extension is properly set up.');
    }
    return this.context;
  }
