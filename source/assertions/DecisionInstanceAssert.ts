...
export class DecisionInstanceAssert extends BaseAssert {
   ...
   */
  async hasResultContaining(expectedPartialResult: Record<string, any>): Promise<this> {
    await this.waitUntil(
      async () => {
        const decision = await this.getDecisionInstance();
  ...
  async hasResultWithLength(expectedLength: number): Promise<this> {
    await this.waitUntil(
      async () => {
        const decision = await this.getDecisionInstance();
        return Array.isArray(decision?.result) && decision.result.length === expectedLength;
      },
